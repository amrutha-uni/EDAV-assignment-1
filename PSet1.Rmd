...
Provide explanation on break points for each and reason out which is better - https://planspace.org/20141225-how_does_r_calculate_histogram_break_points/
...
```{r}
ggplot(mtl, mapping = aes(x=age)) + 
  geom_histogram(binwidth = 5, boundary = 40, closed = c("left")) 

ggplot(mtl, mapping = aes(x=age)) + 
  geom_histogram(binwidth = 5, boundary = 40, closed = c("right")) 

```

Get the borders fixed
...
```{r}
ggplot(mtl, mapping = aes(x=age)) + 
  geom_histogram(binwidth = 5, boundary = 40.5, closed = c("left")) 

ggplot(mtl, mapping = aes(x=age)) + 
  geom_histogram(binwidth = 5, boundary = 40.5, closed = c("right")) 
```

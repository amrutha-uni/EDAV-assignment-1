...
hist(subiculum_thickness_base_r)


ggplot(mtl, aes(x=asubic)) + geom_histogram()
...
2) Then the data and the recommended number of bars gets passed to a function called pretty (usually pretty.default which is implemented in C), which computes a sequence of about n+1 equally spaced round values which cover the range of the values in x. The values are chosen so that they are 1, 2 or 5 times a power of 10. 

  - Calc: Implementation of pretty function is abstracted from the user.             But we know the split is chosen such that the they are 1, 2 or 5           times the power of 10. Base R boxplot shows bin width of size 5.           The value 6 from the above calculation is closest to 5*pow(10, 0)           = 5. Hence we can assume that the pretty function of R                     implemented in C has chosen to multiply 5 with 0th power of 10             and we get a bin width of size 5.
...
```{r}
hist(mtl$age, breaks=seq(min(mtl$age)-1, max(mtl$age) + 1, 5), right=T, main = 'Right closed distribution of Age')
hist(mtl$age, breaks=seq(min(mtl$age)-1, max(mtl$age) + 1, 5), right=F, main = 'Left closed distribution of Age') 
```
...
```{r}
hist(mtl$age, breaks=seq(min(mtl$age) - 0.5, max(mtl$age) + 0.5, 5), right=T, main = 'Right closed distribution of Age')
hist(mtl$age, breaks=seq(min(mtl$age) - 0.5, max(mtl$age) + 0.5, 5), right=F, main = 'Left closed distribution of Age')
```

Strategy - The idea is to change the breakpoints of the histogram to include non integer values such that classifying any integer point will be unambiguous. By specifying the breaks parameter in the hist function to start from a decimal will ensure the boundaries of the bins to be non integers and hence every distribution (right closed or left closed) will be identical. An easy choice is to start off in between an integer, such as 40.5 for instance.
...
* As these norm Q-Q plots for different locations indicate, the "yield" for Brookstead and Lawes are appeared to be more closest to a normal distribution compared with others since their data distribution are more concentrated across the straight line.
...
* Moreover, for the distribution of yield in RedlandBay, there is a right skew since both upper and lower tails for it are at the upper side of the straight line.
...
ggplot(boot::breslow[breslow$smoke=="1",], aes(x=age, y=y)) + 
  ...
  ylab("# of Death") + xlab("age")+
  ggtitle("number of deaths attributed to coronary artery disease among doctors for smokers")
...
ggplot(boot::breslow[breslow$smoke=="0",], aes(x=age, y=y)) + 
  ...
  ylab("# of Death") + xlab("age")+
  ggtitle("number of deaths attributed to coronary artery disease among doctors for non-smokers")
...
ggplot(openintro::loans_full_schema, aes(x=reorder(loan_purpose, loan_amount, median),y = loan_amount)) + 
  ...
  ggtitle("boxplots for loan amount amoung loan purpose")+
  coord_flip()
...
ggplot(openintro::loans_full_schema, aes(x =loan_amount , y = loan_purpose)) +
  ...
  ggtitle("ridgeline plots for loan amount amoung loan purpose")
...
Therefore, boxplot is a better choice for this data.
